...
BG_COLOR = "#fff9c4"  # hellgelb, Notizzettel-Stil

# Label im Filter -> interne Priorität
PRIORITY_VALUE_BY_LABEL = {
    "Rot": "red",
    "Blau": "blue",   # "Blau" im UI, intern blau
    "Grün": "green",
}

# interne Priorität -> Farbe
PRIORITY_COLOR = {
    "red": "#ff4040",
    "blue": "#01B3FA",
    "green": "#28a745",
    "yellow": "#01B3FA",  # alte Daten kompatibel behandeln
}

# Wiederkehrende Tasks
RECURRING_OPTIONS = ["Keine", "Täglich", "Wöchentlich", "Monatlich"]
...
def parse_date(date_str: str):
    """Datum im Format TT.MM.JJJJ oder YYYY-MM-DD zu datetime parsen."""
...
def add_months(dt: datetime, months: int = 1) -> datetime:
    """Monate sauber addieren (für monatlich wiederkehrende Tasks)."""
...
class TaskApp:
    def __init__(self, root):
        ...
        tab_control.add(self.tab_new, text="Neue Task")
        tab_control.add(self.tab_active, text="Aktive Tasks")
        tab_control.add(self.tab_finished, text="Fertige Tasks")
        ...
        # Reminder nach kurzem Delay starten
    ...
    def check_reminders(self):
        """Popup für heute fällige & überfällige Tasks, wiederholt sich selbst."""
        ...
        if overdue or due_today:
            ...
            if overdue:
                ...
                lines += [f"- {t['name']} (Ende: {t['date']})" for t in overdue]
                lines.append("")
            if due_today:
                lines.append("⭐ Heute fällig:")
                lines += [f"- {t['name']} (Ende: {t['date']})" for t in due_today]
            messagebox.showinfo("Erinnerung – fällige Tasks", "\n".join(lines))

        # alle 6 Stunden neu prüfen
    ...
    def build_new_tab(self):
        ...
        ttk.Label(frame, text="Enddatum (TT.MM.JJJJ):").pack(pady=5)
        ...
        ttk.Label(frame, text="Info (Textfeld):").pack(pady=5)
        self.entry_info = tk.Text(frame, height=7, bg="white")
        self.entry_info.pack(pady=5, padx=10, fill="both", expand=True)

        ttk.Label(frame, text="Priorität:").pack(pady=5)
        self.priority = tk.StringVar(value="green")
        prio_frame = ttk.Frame(frame)
        prio_frame.pack()

        ttk.Radiobutton(
            prio_frame, text="Nicht dringend (grün)", variable=self.priority, value="green"
        ).pack(anchor="w")
        ttk.Radiobutton(
            prio_frame, text="Mittel (blau)", variable=self.priority, value="blue"
        ).pack(anchor="w")
        ttk.Radiobutton(
            prio_frame, text="Wichtig (rot)", variable=self.priority, value="red"
        ).pack(anchor="w")

        ttk.Label(frame, text="Wiederholung:").pack(pady=5)
        self.recurrence_var = tk.StringVar(value="Keine")
        ...
        ttk.Button(frame, text="Task speichern", command=self.save_task).pack(pady=10)
    ...
    def save_task(self):
        ...
        if not name or not date_str:
            messagebox.showwarning("Fehler", "Bitte Name und Enddatum eingeben.")
            return
        ...
        try:
            ...
            messagebox.showwarning(
                "Fehler",
                "Bitte Datum im Format TT.MM.JJJJ eingeben (z.B. 20.10.2000).",
            )
            return
        ...
        messagebox.showinfo("Gespeichert", "Task wurde hinzugefügt.")

        self.entry_name.delete(0, tk.END)
        self.entry_date.delete(0, tk.END)
        self.entry_info.delete("1.0", tk.END)
        self.priority.set("green")
        self.recurrence_var.set("Keine")
    ...
    def build_active_tab(self):
        ...
        self.active_sort_order = ttk.Combobox(
            ...
            state="readonly",
            values=[
                "Datum aufsteigend (früh zuerst)",
                "Datum absteigend (spät zuerst)",
            ],
            width=28,
        )
        ...
        ttk.Label(filter_frame, text="Ab Datum:").grid(
        ...
        ttk.Label(filter_frame, text="(TT.MM.JJJJ)").grid(
            row=1, column=2, padx=2, pady=2, sticky="w"
        )

        ttk.Label(filter_frame, text="Suche (Name):").grid(
        ...
        ttk.Button(
            filter_frame, text="Anwenden", command=self.apply_active_manual_filter
        ).grid(row=2, column=3, padx=2, pady=2, sticky="e")

        ttk.Button(
            filter_frame, text="Nur heute", command=self.set_filter_today
        ).grid(row=3, column=0, padx=2, pady=2, sticky="w")
        ttk.Button(
            filter_frame, text="Diese Woche", command=self.set_filter_week
        ...
        ttk.Button(
            btn_frame, text="Task bearbeiten", command=self.edit_active_task
        ).pack(side="left", padx=5)
        ttk.Button(
            btn_frame, text="Task beenden", command=self.finish_task
        ).pack(side="left", padx=5)
    ...
    def update_active_list(self):
        ...
        show_only_overdue = prio_filter_label == "Überfällig"
        if prio_filter_label in ("Alle", "Überfällig"):
        ...
        if date_filter_str:
            ...
            if not filter_date:
                messagebox.showwarning(
                    "Fehler",
                    "Das Filterdatum ist ungültig. Bitte TT.MM.JJJJ verwenden.",
                )
                filter_date = None
        ...
        descending = sort_choice.startswith("Datum absteigend")
    ...
    def show_active_details(self, event):
        ...
        if d is not None and d.date() < today:
            self.details_enddate.config(
                text=f"Ende: {task['date']} – ÜBERFÄLLIG!", fg="red"
            )
        else:
            self.details_enddate.config(text=f"Ende: {task['date']}", fg="black")

        self.details_info.config(text=task["info"])
        created = task.get("created_date", "?")
        rec_label = RECUR_VALUE_TO_LABEL.get(task.get("recurrence"), "Keine")
        self.details_created.config(
            text=f"Erstellt am: {created} | Wiederholung: {rec_label}"
        )
    ...
    def get_selected_active_task(self):
        ...
        if not selection:
            messagebox.showwarning(
                "Hinweis", "Bitte zuerst eine aktive Task auswählen."
            )
            return None, None
        ...
        try:
            ...
            messagebox.showerror("Fehler", "Task konnte nicht gefunden werden.")
            return None, None
    ...
    def edit_active_task(self):
        ...
        edit_win.title("Task bearbeiten")
        ...
        ttk.Label(edit_win, text="Enddatum (TT.MM.JJJJ):").pack(pady=5)
        ...
        ttk.Label(edit_win, text="Priorität:").pack(pady=5)
        ...
        ttk.Label(edit_win, text="Wiederholung:").pack(pady=5)
        rec_var = tk.StringVar(
            value=RECUR_VALUE_TO_LABEL.get(task.get("recurrence"), "Keine")
        )
        rec_combo = ttk.Combobox(
            edit_win,
        ...
        def save_changes():
            ...
            if not new_name or not new_date_str:
                messagebox.showwarning(
                    "Fehler", "Bitte Name und Enddatum eingeben."
                )
                return
            ...
            try:
                ...
                messagebox.showwarning(
                    "Fehler", "Bitte Datum im Format TT.MM.JJJJ eingeben."
                )
                return
        ...
        ttk.Button(edit_win, text="Änderungen speichern", command=save_changes).pack(
            pady=10
        )
    ...
    def finish_task(self):
        ...
        messagebox.showinfo("Erledigt", "Task wurde abgeschlossen.")
    ...
    def build_finished_tab(self):
        ...
        ttk.Label(filter_frame, text="Ab Datum (fertig):").grid(
            row=1, column=0, padx=2, pady=2, sticky="w"
        )
        ...
        ttk.Button(filter_frame, text="Anwenden", command=self.update_finished_list).grid(
    ...
    def update_finished_list(self):
        ...
        for t in self.sorted_finished_tasks:
            display_text = f"↩ ● {t['name']} – beendet am {t.get('finished_date', '?')}"
    ...
    def show_finished_details(self, event):
        ...
        self.finished_enddate.config(text=f"Ende: {task['date']}")
        ...
        self.finished_extra.config(
            text=f"Erstellt am: {created} | Abgeschlossen am: {finished} | Wiederholung: {rec_label}"
        )
    ...
    def reactivate_finished_task(self):
        ...
        if not selection:
            messagebox.showwarning(
                "Hinweis", "Bitte zuerst eine fertige Task auswählen."
            )
            return
        ...
        self.update_finished_list()
        self.update_active_list()
        messagebox.showinfo("Reaktiviert", "Task wurde wieder als aktiv gesetzt.")
...
def run_tray(app: TaskApp):
    ...
    icon = pystray.Icon("NotizZettel", image, "Notiz Zettel", menu)
    app.tray_icon = icon
    icon.run()
